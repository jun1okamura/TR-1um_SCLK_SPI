"""
route_gio_core_v10.py (design_notes.md 108.55, user instruction "一旦
コミットして、チップレベルの配線に進んでください" -- V10's chip-level
GIO<->core routing, adapted from route_gio_core_v9.py)

Draws the 20 signal nets (addr_match/busy/rw/rx_valid intentionally
unbonded, matching V7/V9 precedent) + VDD/GND onto
src/tr_1um_i2c_slave_async_v10.gds (V10's core+GIO+PTECT placement, from
script/assemble_top_v10.py, core at V9's exact CORE_OFFSET per explicit
user instruction), reusing v9's ring-routing/lane-packing engine
UNCHANGED (perimeter_s/s_to_xy/ring_waypoints/project_to_R/seg_layer,
R_NOM=880/LANE_R0=847/LANE_PITCH=6.0/NEAR_R=913.0/DIS_R=840.0/
VDD_PRIVATE_R=834.0), with V10-specific inputs:

  - Signal net core-side coordinates: V10's REAL GDS-extracted pin
    positions (schematic/v10_signal_routing_plan.json).
  - Signal net GIO-side coordinates: 108.54 found V10's pin-edge
    distribution differs sharply from V9's (e.g. all 8 rx_data bits now
    exit LEFT instead of split RIGHT/LEFT) -- reusing V9's net-name-
    preserving pad map needs 14 lanes (exceeds the ~11-12 lane budget,
    NOT routable). schematic/v10_signal_routing_plan.json instead reuses
    V9's same 20 fixed physical GIO pad POSITIONS but with the net->pad
    mapping chosen by an optimal (scipy linear_sum_assignment) minimum-
    total-unrolled-interval-width pairing -- needs only 6 lanes.
  - Direction (CW/CCW) and lane assignment: recomputed fresh from V10's
    positions via the identical greedy-interval-scheduling algorithm --
    6 lanes (well within budget, even fewer than V9's own 10-11).
  - VDD/GND: SAME bus-bar architecture as v9 (TAP-column pins -> M1/M2
    bus bars -> real GIO VDD/VSS LEF pins). VSS (bottom) side geometry is
    UNCHANGED from v9 -- V10's core bottom edge is at the same chip Y
    (-140.0) as v9's, since CORE_OFFSET is identical and both cores'
    native bbox bottom is y=0. VDD (top) side needed one recalibration:
    V10's core is 2.8um taller (native H=938.9 vs 936.1), so
    NATIVE_TOP_Y/TOP_Y are shifted +2.8um to match V10's actual top-
    margin native power strap position; VDD_BUS_Y/CROSS_Y/GIO pin
    targets are frame-fixed and unchanged (still enough clearance --
    V10's signal lanes top out at R=877, well below v9's ~907, so if
    anything there's MORE headroom for the VDD-pin crossing than v9 had).
  - DIS chain (P7 + 8 HIZ pins, GIO-pin-to-GIO-pin only, no core
    involvement): reused VERBATIM -- entirely a property of the frame,
    unaffected by which core is wired in.

This is a FIRST PASS, not yet DRC-verified -- v9's own equivalent
(route_gio_core_v9.py) needed several real-DRC-driven revision rounds
(see its own extensive comments, 79.4-79.12) even reusing frame-fixed
geometry; V10 additionally has an entirely new (untested) net->pad
mapping. Output is a draft GDS for drc_check_nrow_fm.py / connectivity
verification, iterate from there.
"""
import json
import sys
...
IN_GDS = BASE + "/src/tr_1um_i2c_slave_async_v10.gds"
OUT_GDS = BASE + "/layout/step10/v10_top_routed.gds"
TOP_CELL = "tr_1um_i2c_slave_async"  # same cell name as v9's inside the GDS (assemble_top_v10.py's TOP_CELL_NAME); only the file path differs

SIGNAL_PLAN = BASE + "/schematic/v10_signal_routing_plan.json"
...
M2_WIRE_W = 3.4
PAD = 3.4
# LANE_PITCH/NEAR_R REVISED (108.57, pad-pairing fix): preserving the real
# bidirectional pad architecture (tx_data[i]/rx_data[i] sharing one
# physical pad, sda_in/sda_oe sharing P2/HIZ2 -- see
# assign_v10_gio_pads.py) needs 12 lanes, an exhaustively-verified
# minimum under that constraint (80640 valid pairings checked via brute
# force, none needs fewer). At the original LANE_PITCH=6.0/NEAR_R=913.0,
# the 12th (top) lane's own R is EXACTLY 847+11*6=913.0 -- a zero-length
# hop to NEAR_R, the exact degenerate case route_gio_core_v9.py's own
# comments warn crashes with a ValueError. Neither LANE_PITCH nor NEAR_R
# is a hard physical limit (only M2 Smin=2.0 and the real GIO pin-
# routing boundary 921.7 are) -- narrowed LANE_PITCH to 5.6 (M2_WIRE_W
# 3.4 + 2.2um gap between adjacent lanes, still above Smin=2.0 with a
# small margin, not the bare-minimum 5.4) so 12 lanes only need
# 847..908.6 (top lane outer edge 910.3), leaving real room for NEAR_R:
# set to 915.0 (inner edge 913.3, 3.0um clear of the top lane; outer edge
# 916.7, 5.0um clear of the real 921.7 pad boundary).
LANE_R0 = 847.0
LANE_PITCH = 5.6
NEAR_R = 915.0
R_NOM = 880.0
...
# FORCE_DIR (108.57 fix): V9's rx_data[1]-forces-CW override was tied to
# v9's specific net->pad mapping; doesn't carry over to V10's own mapping
# (108.57's terminal-type-constrained reassignment).
#
# A REAL BUG was found here instead: the default "shortest arc"
# direction pick (CW if d_cw<=d_ccw else CCW, computed from the RAW,
# non-unrolled s values) can silently disagree with the lane-packing
# interval math further down, which always assumes a net's physical
# footprint is the SIMPLE, non-wrapping range
# (min(unroll(s1),unroll(s2)), max(unroll(s1),unroll(s2))). That
# assumption is only true if the path actually taken is the one that
# stays within [u1,u2] without wrapping across the u=0/PERI seam -- true
# for CW when u1<u2, or CCW when u1>u2. If the "shortest arc" pick
# disagrees with this (chooses the WRAPPING direction because it happens
# to be a shorter physical distance), the lane-packing algorithm
# computes an interval that does NOT bound the net's real footprint,
# and can silently pack two nets whose true paths physically overlap
# into the same lane. Found empirically this session: rx_data[2] (core
# LEFT edge -> OUT11 on RIGHT, terminal-type-constrained assignment,
# 108.57) naturally picked CCW as the "shorter" 3380um arc, wrapping
# through u=0 -- but its lane-packing interval was computed as the
# NON-wrapping (2015.3,6035.0), a 4019.7-wide span that does not
# describe where the wire actually is. rx_data[7] also ended up on the
# same lane (R=847) with an interval that, by chance, did NOT overlap
# (2015.3,6035.0) on paper -- but the ACTUAL wires collided (a same-
# layer M2 short, confirmed by direct klayout.db overlap query, area
# 3685.9um^2) because rx_data[2]'s real (wrapped) footprint extends
# through the exact region rx_data[7]'s wire occupies.
#
# Fix: force EVERY net's direction to whichever one keeps its physical
# footprint inside the simple [min(u1,u2),max(u1,u2)] range the lane-
# packing algorithm assumes -- CW when u1<u2, CCW when u1>u2 -- instead
# of trusting the "shortest arc" heuristic. This sometimes picks the
# longer of the two physical arcs (e.g. rx_data[2] now takes the
# 4019.7um CW arc instead of its shorter 3380.3um CCW one), but
# guarantees the lane-packing math is actually correct, which matters
# far more than shaving a few hundred um off one wire's length.
def _forced_direction(core, gio):
    s1 = perimeter_s(core["x"], core["y"], core["edge"], R_NOM)
    s2 = perimeter_s(gio["x"], gio["y"], gio["edge"], R_NOM)
    u1, u2 = unroll(s1), unroll(s2)
    return "CW" if u1 < u2 else "CCW"
...
#
# HIZ13 INSERTED (108.59, real LVS run found OSS_FRAME_GIO NoMatch --
# 8 layout-side pins missing entirely: HIZ7/9/10/13/15/OUT2/P9/P10).
# gio_connections.json's CURRENT per-terminal detail documents HIZ13 as
# "moved here from being sda_oe's dynamic control -- now a plain
# DIS-chain member" (freed up by the HIZ2<->HIZ13 swap that made HIZ2
# sda_oe's dynamic control instead -- HIZ2 already routes correctly as
# one of the 20 real signal nets, confirmed matching in the LVS run).
# This DIS_CHAIN_POINTS list was inherited verbatim from an EARLIER v9
# state that predates that swap (when HIZ13 was still sda_oe's own
# point, not a chain member) and was never updated. HIZ13's own position
# (921.7,220.0,RIGHT) sits exactly between HIZ14(921.7,580.0) and
# HIZ12(921.7,-220.0) on the same RIGHT edge, so it's inserted as a new
# link in that exact spot (HIZ14_HIZ13, HIZ13_HIZ12 replacing the single
# HIZ14_HIZ12 link) rather than appended elsewhere.
DIS_CHAIN_POINTS = [
    # HIZ1 REMOVED (108.63): a real LVS run (layout/step10/LVS_error.
    # lvsdb, after 108.62 unblocked the top-level comparison) found P7's
    # net with an extra (10th) subcircuit pin vs. the reference's 9 --
    # gio_connections.json's own connections_per_terminal_detail marks
    # HIZ1 as `{"net": "VDD", "meaning": "Hi-Z / input-only (moved here
    # from HIZ2)"}`, NOT a DIS-chain member. This DIS_CHAIN_POINTS list
    # (like HIZ13's own omission, fixed in 108.59) was inherited from an
    # earlier v9 state that predates the HIZ2->HIZ1 VDD-tie reassignment
    # and was never updated -- HIZ1 was left wired into the DIS chain
    # instead of getting its own VDD tie. Fixed here (removed from the
    # chain -- it was the chain's own start/endpoint, so no bridging
    # link is needed) and given a real VDD tie below (HIZ_VDD_VSS_TIES).
...
def main():
    ...
    # ---- build NETS dict for the 20 signal nets from the plan ----
    NETS = {}
    for name, e in sig_plan["nets"].items():
        c, g = e["core"], e["gio"]
        fd = _forced_direction(c, g)
    ...
    # TAP X positions unchanged from v9 -- horizontal row/TAP-column tiling
    # (native cell X placement) did not change between v9 and v10, only
    # core HEIGHT did (row content/count), and CORE_OFFSET_X is identical.
    VDD_TAP_X = {"TAP0": -801.9, "TAP1": -267.3, "TAP2": 267.3, "TAP3": 807.3}
    VSS_TAP_X = {"TAP0": -807.3, "TAP1": -272.7, "TAP2": 261.9, "TAP3": 801.9}
    # TOP_Y (v10, 108.55): v9's value (794.6) was NATIVE_TOP_Y(796.1)-1.5;
    # only used for the informational pwr_log print below, not for any
    # geometry -- shifted by the same +2.8um core-height delta so the log
    # stays accurate. BOTTOM_Y unchanged -- v10's core bottom sits at the
    # same chip Y (-140.0) as v9's (identical CORE_OFFSET, both cores'
    # native bbox bottom = 0).
    TOP_Y = 797.4
    BOTTOM_Y = -138.5
    VDD_BUS_Y = 800.0
    VSS_BUS_Y = -805.0
    ...
    # NATIVE_TOP_Y (v10, 108.55): v9's core top-margin native power strap
    # top edge was 796.1 = CORE_OFFSET_Y(-140.0) + v9 native core H(936.1).
    # v10's native core H is 938.9 (108.52's re-squeeze result), so its own
    # top-margin native strap sits 2.8um higher: -140.0+938.9 = 798.9.
    NATIVE_TOP_Y = 798.9
    ...
    gx, gy = GIO_VDD_PIN
    # CROSS_Y REVISED (108.57): v9's value (910.0) assumed v9's own lane
    # ceiling (~907); V10's pad-pairing-constrained 12-lane layout now
    # tops out at R=908.6 (LANE_PITCH=5.6, top lane outer edge ~910.3).
    # First attempt moved this to 912.0, but the via's own M1 landing pad
    # (PAD=3.4, so spanning CROSS_Y+/-1.7) then started at Y=910.3 --
    # touching the top lane's own M1 edge with ZERO gap, a real M1.S1
    # space violation (Smin=1.4) caught by drc_check_nrow_fm.py. Moved to
    # 914.5: via pad now spans 912.8-916.2, a clean 2.5um gap below
    # (912.8-910.3) and confirmed (direct klayout.db query) nothing else
    # occupies X=[190,210] between the pad's own top edge (916.2) and the
    # real GIO_VDD_PIN region starting at Y~920.
    CROSS_Y = 914.5
    ...
    # 5-STRIP WIDENING (108.64, user request): "VSS/VDDのM1バーと電源PAD
    # を繋いでいる10umの配線は、原状の配線と同じモノを、左右２本づつ、
    # 計５本になるように２umスペースをおいて追加してください" -- 5
    # parallel copies of this same 10um-wide tie (pitch = width(10) +
    # space(2) = 12um center-to-center: gx-24, gx-12, gx, gx+12, gx+24),
    # for extra current-carrying margin. Verified via klayout.db
    # (recursive, so it actually sees content nested inside the
    # OSS_FRAME_GIO instance) that the wider crossing corridor
    # (X=gx-24..gx+24 = 176..224, Y=800..914.5) contains nothing besides
    # this tie's own existing shapes -- the nearby OSS_FRAME_GIO shapes
    # at X=150-190/210-250 spanning Y=920-990 (just above CROSS_Y, so
    # never entered by this crossing) are themselves part of the VDD
    # net's own bond-pad structure (same cell family as the confirmed-
    # VDD "OSS_ESD_5V_VDD"/"OSS_PAD" shapes directly below them at
    # Y=920-950 spanning the full X=0-400 -- i.e. this whole region is
    # VDD's own pad, not a different net), so landing 5 strips across it
    # is safe, not a short.
    GX_OFFSETS = (-24.0, -12.0, 0.0, 12.0, 24.0)
    for i, dx in enumerate(GX_OFFSETS):
        sx = gx + dx
        multi_via(sx, VDD_BUS_Y)                          # M1 bus -> M2, full BUS_W on both sides now
        bwire("M2", sx, VDD_BUS_Y, sx, CROSS_Y)            # full 10um crossing of the signal-lane band
        multi_via(sx, CROSS_Y)                             # M2 -> M1, full BUS_W on both sides
        bwire("M1", sx, CROSS_Y, sx, gy)                   # M1 (10um) the rest of the way into the real pin
        pwr_log.append((f"VDD_GIO_pin_{i}", sx, gy))
    ...
    gx, gy = GIO_VSS_PIN
    # 5-STRIP WIDENING (108.64, same user request/rationale as VDD_GIO_pin
    # above): the real VSS pin rect is (-450,-934)-(50,-920), 500um wide,
    # comfortably covering all 5 offset X's; the whole X=-400..0,
    # Y=-950..-920 area at this pad is confirmed VSS's own bond-pad/ESD
    # structure (cell "OSS_ESD_5V_VSS"), so landing 5 parallel strips
    # across it is safe.
    GX_OFFSETS = (-24.0, -12.0, 0.0, 12.0, 24.0)
    for i, dx in enumerate(GX_OFFSETS):
        sx = gx + dx
        bwire("M2", sx, VSS_BUS_Y, sx, gy)
        multi_via(sx, VSS_BUS_Y)  # M2 stub -> M1 bus junction -- was missing (an M1 box and M2
        # box merely touching at the same point isn't an electrical connection without a via).
        pwr_log.append((f"VSS_GIO_pin_{i}", sx, gy))
    ...
    # ---- static VDD/VSS ties (108.59) ----
    # gio_connections.json's connections_per_terminal_detail marks these
    # 5 GIO terminals as fixed ties (not core signals, not DIS-chain
    # members): HIZ7/HIZ15 -> VDD ("Hi-Z / input-only"), HIZ9/HIZ10 ->
    # VSS ("output driver ENABLED", RING_OSC's own OUT9/OUT10 direction
    # control), OUT2 -> VSS ("permanently tied low -- open-drain SDA
    # drive data"). v9's own final chip drew these (script/add_hiz_vss_
    # ties_v9.py, restoring 6 ties an earlier v9 power-bus rewrite had
    # dropped -- design_notes.md 83.x/84.x) but route_gio_core_v10.py
    # never carried an equivalent step over, so all 5 (plus OUT2, whose
    # own tie moved here from the historical OUT13 in a later v9
    # reassignment) were left as bare floating M2 pin stubs in every V10
    # build -- confirmed via a real KLayout LVS run (layout/step10/
    # LVS_error.lvsdb, 108.59): OSS_FRAME_GIO NoMatch, exactly these 5
    # (+HIZ13, fixed above, +P9/P10 spares) reported with no layout-side
    # pin at all.
    #
    # All 5 targets are LEF-declared M2 pins (LEF/OSS_FRAME_GIO.lef,
    # verified directly -- HIZ7/HIZ9/HIZ10/HIZ15/OUT2 all `LAYER M2`),
    # each a 3.4x3.4 stub sitting exactly at NEAR_R=915..921.7's outer
    # edge (the same real bond-pad ring radius as every signal net's own
    # final hop). The VDD/VSS bus bars sit deep inside the die (Y=800/
    # -805) -- reaching a pad out at Y=+-921.7 means crossing the entire
    # 12-lane signal band (Y~=847..908.6) and the DIS chain's own private
    # ring (R=840), ~120um of radial distance. A systematic klayout.db
    # column sweep (X=<pin's own X> +-3um, full Y span from the bus to
    # the pad) confirmed ZERO M2 shapes anywhere in any of the 5 columns
    # (only M1 lane-travel wires cross through, which a plain M2 vertical
    # wire clears safely via the same different-layer-crossing convention
    # used throughout this project -- no via, no short) -- so each tie is
    # simply a straight M2 wire from the bus to the pin, no lane-weaving
    # needed. Verified via drc_check_nrow_fm.py + same-layer overlap
    # re-check after implementation (see design_notes.md 108.59).
    # BUS-TARGET BUG FIXED (108.63): the original 108.59 fix connected
    # HIZ7 and OUT2 to whichever bus was PHYSICALLY NEARER their own pad
    # (both happened to sit on the "wrong" edge relative to their real
    # target net -- HIZ7's pad is on the BOTTOM edge, near VSS_BUS_Y, but
    # gio_connections.json says HIZ7 -> VDD; OUT2's pad is on the TOP
    # edge, near VDD_BUS_Y, but gio_connections.json says OUT2 -> VSS,
    # "permanently tied low"). A real LVS run (after 108.62 unblocked the
    # top-level comparison) confirmed this directly: LAYOUT net VDD
    # contained OUT2 (should not), LAYOUT net VSS contained HIZ7 (should
    # not) -- swapped versus the reference.
    #
    # HIZ15/HIZ1/HIZ9/HIZ10 stay simple same-side ties (their pad and
    # target bus are on the same edge, ~120um straight M2 hop). HIZ7/
    # OUT2 now need to reach the FAR bus, crossing almost the full die
    # height. TWO earlier attempts at this were tried and rejected:
    #  1st: a plain straight vertical wire at the pin's own X -- failed
    #    real DRC (M2 width=1/space=9 new violations). The CORE cell's
    #    own internal M2 routing occupies nearly the entire die
    #    interior (i2c_slave_async_nrow_fm's own standard-cell wiring,
    #    confirmed via klayout.db.begin_shapes_rec() -- a non-recursive
    #    shape query misses instance-nested content entirely, which is
    #    why the original pre-check missed this).
    #  2nd: a short local jog (~30um) to a "clear" X near the pin, still
    #    inside the core's own X footprint -- a systematic per-X-column
    #    scan (klayout.db, recursive, full margin including the 2.0um
    #    DRC clearance on both sides, not just the wire's own half-
    #    width) showed the core's internal routing is packed densely
    #    enough (~5-6um row pitch) that NO column anywhere within its
    #    footprint (X roughly -680..-480) clears with proper margin --
    #    every candidate had a real conflict within 2um.
    # FIXED (this revision, 3rd attempt -- cartesian corridor abandoned):
    # attempt 2 (X=790/797, ~7um apart) still failed real DRC -- a proper
    # full-margin (X +-3.7um, M2 Wmin=3.0/Smin=2.0) recursive-shape
    # recheck found X=790 clips real core-internal M2 wiring at
    # X=784.0-787.4 (only 0.9um clear, need 2.0) and X=797 clips a
    # TAP2/TAP3 well-tap standard-cell column at X=800.2-803.6 (also
    # <2.0um clear). Retargeting to the ONLY genuinely-clear ~12.8um gap
    # on each side (787.4-800.2 / -800.2--787.4, fits exactly one 3.4um
    # wire, not two -- so HIZ7 got X=-794, OUT2 got the mirrored X=+794
    # via a long M1 jog) passed DRC against v10_top_routed.gds alone,
    # but finalize_chip_v10.py's LATER additions -- RING_OSC (a full-
    # width standard-cell row placed at Y=-768.9..-645.8, blocking EVERY
    # X from -798.2 to +708.4) and OPENSUSI_LOGO (a pixel pattern
    # spanning X=-791.5..791.5 at Y=-493.4..-180.4) -- both clip these
    # same X=-+-794 corridors once stacked on top (confirmed via a
    # recursive-shape recheck of v10_chip_final.gds, not just
    # v10_top_routed.gds -- the earlier checks against the pre-finalize
    # GDS were too early to see these). With RING_OSC alone blocking a
    # SOLID 1506um-wide band (nearly the entire die interior) at that Y,
    # no cartesian N-S corridor through the die interior survives at any
    # X between the TAP columns (|X|~800) -- confirmed by direct
    # inspection (RING_OSC's own power/ground straps merge into one
    # unbroken shape, no exploitable gap).
    #
    # FIXED: abandoned the interior-corridor approach entirely in favor
    # of this project's OWN established mechanism for exactly this kind
    # of GIO-pin-to-GIO-pin "wraparound" routing -- the same
    # ring_waypoints()/connect_gio_to_gio() engine already proven by the
    # DIS chain (DIS_R=840, connects P7 + 8 HIZ pins around the die's
    # OUTER perimeter, between the core/RING_OSC/LOGO interior and the
    # pad ring, entirely clear of both). Rather than routing HIZ7/OUT2
    # all the way to the bus in cartesian coordinates, each is ring-
    # routed to an ALREADY-bus-tied pin on the opposite edge instead:
    # HIZ7 (BOTTOM, VDD-target) rides the ring to HIZ1 (TOP, already
    # VDD-tied via HIZ_VDD_VSS_TIES below); OUT2 (TOP, VSS-target) rides
    # the ring to HIZ9 (BOTTOM, already VSS-tied). Physically overlapping/
    # via-joining onto an already-correctly-tied pin's own metal is
    # electrically identical to reaching the bus directly -- HIZ1 and
    # HIZ9 carry nothing but VDD/VSS themselves (permanently-tied spare
    # Hi-Z pins, gio_connections.json), so joining onto them cannot
    # introduce a new node. Uses NEW private ring radii, radially inside
    # DIS_R=840 with full M2 Wmin/Smin clearance (5.4um needed between
    # concentric ring centers): HIZ7_VDD_RING_R=834.0 (already defined,
    # matches v9's own VDD_PRIVATE_R constant, 6um clear of DIS_R) and
    # OUT2_VSS_RING_R=828.0 (6um further in, clear of both DIS_R and the
    # HIZ7 ring). Both radii stay well clear of the TAP columns (|X|~800)
    # and RING_OSC/LOGO (both confined well inside |X|,|Y|<800) since
    # they trace a square 828-834um out from center, same territory as
    # the already-DRC-clean DIS chain. Verified 0 new DRC violations
    # end-to-end after implementing (see design_notes.md 108.63).
    HIZ7_VDD_RING_R = VDD_PRIVATE_R  # 834.0
    OUT2_VSS_RING_R = 828.0

    HIZ_VDD_VSS_TIES = [
        # (label, pin_x, pin_y, bus_y, bus_layer_y_span_center)
        ("HIZ15_VDD_tie", 580.0, 923.4, VDD_BUS_Y),
        ("HIZ1_VDD_tie", -220.0, 921.7, VDD_BUS_Y),
        ("HIZ9_VSS_tie", 220.0, -923.4, VSS_BUS_Y),
        ("HIZ10_VSS_tie", 580.0, -923.4, VSS_BUS_Y),
    ]
    for label, px, py, bus_y in HIZ_VDD_VSS_TIES:
        cur_net[0] = label
        via(px, bus_y)  # M1 bus -> M2 transition
        # extend ~2um past the pin's own near edge (920.0/-920.0) into
        # its LEF rect (920.0-923.4 or -923.4--920.0) for a solid,
        # non-edge-only overlap, matching this project's established
        # "overlap by margin, don't just touch" convention.
        pin_near_edge = 922.0 if py > 0 else -922.0
        wire(m2_idx, px, bus_y, px, pin_near_edge, M2_WIRE_W)
        print(f"{label:<16} x={px:7.1f} y=[{bus_y:.1f},{pin_near_edge:.1f}]")

    # ---- HIZ7 -> VDD, OUT2 -> VSS: far-bus crossing ties (108.63) ----
    # Ring-routed (see comment above) onto an already-tied pin on the
    # opposite edge, reusing connect_gio_to_gio() (defined above, same
    # engine as the DIS chain) instead of a cartesian interior corridor.
    HIZ7_PIN  = (-580.0, -923.4, "BOTTOM", "M2")
    OUT2_PIN  = (-620.0,  923.4, "TOP",    "M2")
    HIZ1_PIN  = (-220.0,  921.7, "TOP",    "M2")   # already VDD-tied below
    HIZ9_PIN  = ( 220.0, -923.4, "BOTTOM", "M2")   # already VSS-tied below

    nv = connect_gio_to_gio("HIZ7_VDD_tie", HIZ7_PIN, HIZ1_PIN, HIZ7_VDD_RING_R)
    print(f"{'HIZ7_VDD_tie':<16} ring R={HIZ7_VDD_RING_R:.1f} HIZ7->HIZ1(VDD) vias={nv}")
    nv = connect_gio_to_gio("OUT2_VSS_tie", OUT2_PIN, HIZ9_PIN, OUT2_VSS_RING_R)
    print(f"{'OUT2_VSS_tie':<16} ring R={OUT2_VSS_RING_R:.1f} OUT2->HIZ9(VSS) vias={nv}")
