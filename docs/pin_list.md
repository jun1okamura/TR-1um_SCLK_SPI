...
- **DATA 8本** … `HIZ` は **DIS パッド(`P5`)のネットに直結**。コアがDATAを駆動するのは `data_oe = ~dis = 1`、つまり `dis = 0` のときで、`HIZ = dis` が過不足なく一致するので論理が要らない
...
- **`data_oe`** … `~dis` と同じ情報で、これを使うパッドはすべて `HIZ` を DIS パッドのネットから取っている。プローブ用にポートとしては残してある
