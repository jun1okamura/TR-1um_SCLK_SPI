...
STA_FALSE_PATH_FROM = ["rstn"]                # 非同期リセット
...
#   帯の上端 437.2 -> コア下端 457.2、コア高 319.0 で**コア上端 776.2**。
#   GND バスの M1 下端 781.0 と 4.8 µm 空く。
# ★ **コア高が変わったらここを動かす**（U32、2026-09-17）。上は詰まっていて
#   逃がせない — レーンは 13 本要り、`CHIP_LANE_R0 = 810.0` が最上 874.8 で
#   GND リングの上限 875.3 に 0.5 µm しか余っていない。だから**コアの下端**を
#   下げて吸収する。ロゴの帯は -800…404.8 = 1204.8 µm あり、要るのは 730 µm。
#   旧 415.6（コア高 340.6 のとき）。合成し直してコアが 319.0 になったので
#   **コア上端が 776.2 に来るように**上げた。
#   `config_base.chip_stack()` が「コア上端 -> GND バス下端」で見ている。
LOGO_BOX = (-795.0, -800.0, 795.0, 437.2)
LOGO_SCALE = 1                                # 等倍
...
PLACE_SEED = 3                                # 掃引して選んだ（U32、2026-09-17）
# ★ **選び方**: 1〜6 を振って、(1) チップの電源ストリップ束（`VDD`/`GND`/`VSS`
#   各 5 本）が**コア上辺のポートと重ならない**こと（`route_chip.py` の U41 の
#   検査）と、(2) 圧縮後のコア高、の 2 つで見た。
#
#     seed 1  VDD NG / GND ok / VSS ok   コア 1611.0 x 351.4
#     seed 2  VDD NG（tx_data[7] が 224.1）                  ―
#     seed 3  **3 つとも ok**            コア 1611.0 x **319.0**   <- これ
#     seed 4  VDD NG / GND NG            コア 1611.0 x 340.6
#     seed 5  VDD NG / GND NG            コア 1611.0 x 351.4
#     seed 6  VDD ok / GND NG            コア 1611.0 x 340.6
#
#   seed 3 は**唯一 3 つとも通り、かつコアがいちばん低い**。
#   旧 `PLACE_SEED = 1`（合成し直す前の版で DRC 0 / LVS Match /
#   ngspice 12/12 が通っていた値。振っていなかった）。
